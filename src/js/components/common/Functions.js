...
export const choroplethColorArray = (stops, color, firstColor) => {
  ...
  var gradient = gradStop({
    stops: stops,
    inputFormat: 'hex',
    colorArray: [firstColor ? firstColor : '#b3f0f4', color],
  });
  return gradient;
};
...
export const label_Vector_Tiles = (
  feature,
  vt_label_province,
  labelcount,
  provinceCounts,
  ...
  if (feature.properties.hasOwnProperty('PROV_NAME')) {
    ...
    var size = getProvinceCircleSize(
      provinceCounts[feature.properties.id],
      max,
      min,
    );
    ...
    var divIconP = L.divIcon({
      html:
        ...
        marginLeft +
        "'><span id= 'provinceCount" +
        feature.properties.id +
        "' class='text_circle' style ='margin-left:-2px; line-height:" +
        size +
        "px;'>" +
        provinceCounts[feature.properties.id] +
        "</span></div><text class='fed_labelText' style='color:#383838; font-size: 10px; margin-left: -10px;'>" +
        name +
        '</text>',
    ...
    vt_label_marker_P.bindTooltip(
      '<div>Number of Automation Deployed</div>',
    );
...
export const makeZeroLastNumber = num => {
  return parseInt(num.toString().replace(/.$/, '0')) > 12
    ? parseInt(num.toString().replace(/.$/, '0'))
    : 12; //replace last digit by zero
};
...
export const getProvinceName = (id, language) => {
  ...
  if (id === 3) {
    ...
    ProvinceName =
      language == 'en' ? 'Sudurpashchim' : 'सुदूरपश्चिम प्रदेश';
  } else if (id === 1) {
...
export const countEqual = (oo, pp, labelcount) => {
  ...
  Object.keys(oo).map(key => {
    ...
    if (
      oo[key]._latlng.lat == pp._latlng.lat &&
      oo[key]._latlng.lng == pp._latlng.lng
    ) {
      labelcount++;
    }
